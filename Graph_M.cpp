...
public:
    ...
    void display_Map() {
        ...
        for (const auto& kv : vtces) {
            cout << kv.first << " =>\n";
            for (const auto& nbr : kv.second.nbrs) {
...
public:
    int dijkstra(string src, string des, bool nan) {
        ...
        for (const auto& kv : vtces) {
            ...
            dp.vname = kv.first;
            dp.psf = "";
            dp.cost = (kv.first == src) ? 0 : INT_MAX;
            pq.push(dp);
            map[kv.first] = dp;
        }
    ...
    string Get_Minimum_Distance(string src, string dst) {
        ...
        deque<pair<string, int>> stack;

        stack.push_front({src, 0});

        while (!stack.empty()) {
            auto rp = stack.front();
            stack.pop_front();

            if (processed.find(rp.first) != processed.end()) {
                continue;
            }

            processed[rp.first] = true;
            ...
            for (const auto& nbr : vtces[rp.first].nbrs) {
                if (processed.find(nbr.first) == processed.end()) {
                    stack.push_front({nbr.first, rp.second + nbr.second});
                }
            }
        }
    ...
    string Get_Minimum_Time(string src, string dst) {
        ...
        while (!stack.empty()) {
            ...
            for (const auto& nbr : vtces[rp.first].nbrs) {
                if (processed.find(nbr.first) == processed.end()) {
                    stack.push_front({nbr.first, rp.second + 120 + 40 * nbr.second});
                }
            }
        }
...
void Create_Metro_Map(Graph_M& g) {
    ...
    g.addEdge("Saket~Y", "AIIMS~Y", 6);
    g.addEdge("AIIMS~Y", "Rajiv Chowk~BY", 7);
    g.addEdge("Rajiv Chowk~BY", "Vishwavidyalaya~Y", 9);
    g.addEdge("Rajiv Chowk~BY", "Chandni Chowk~Y", 1);
    g.addEdge("Chandni Chowk~Y", "New Delhi~YO", 2);
    g.addEdge("New Delhi~YO", "Shivaji Stadium~O", 3);
    g.addEdge("Shivaji Stadium~O", "DDS Campus~O", 6);
    g.addEdge("DDS Campus~O", "IGI Airport~O", 8);
    g.addEdge("Janak Puri West~BO", "Rajouri Garden~BP", 6);
    g.addEdge("Rajouri Garden~BP", "Netaji Subhash Place~PR", 7);
    g.addEdge("Netaji Subhash Place~PR", "Punjabi Bagh West~P", 3);
}
...
int main() {
    ...
    string src = "Noida Sector 62~B";
    string dst = "Dwarka Sector 21~B";

    cout << "Minimum Distance: " << g.Get_Minimum_Distance(src, dst) << " km\n";
    cout << "Minimum Time: " << g.Get_Minimum_Time(src, dst) << " minutes\n";
